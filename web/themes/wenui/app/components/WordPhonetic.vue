...
import phoneticsConfig from '@/config/phonetics.json'
...
const { getIrregularWord } = useWordService()
...
function getPhoneticFromConfig(wordId) {
  ...
  // phoneticsConfig 是一个对象，key 是 wordId 的字符串形式
...
async function getPhonetic(word, wordId, cached) {
  ...
  // 第一级：从当前字段获取（cachedPhonetic 参数）
  if (cached) {
    console.log(`✅ [第一级] 使用当前字段的音标: ${word} -> ${cached}`)
    return cached
  }

  // 第二级：从配置文件 phonetics.json 通过 wordId 获取
