...
onMounted(async () => {
  try {
    // 加载设置
    const settingsResponse = await axios.get('http://localhost:3123/api/settings')
    if (settingsResponse.data?.quiz?.questionsPerPage) {
      questionsPerPage.value = settingsResponse.data.quiz.questionsPerPage
    }

    const response = await axios.get('http://localhost:3123/api/reading-comprehension')
